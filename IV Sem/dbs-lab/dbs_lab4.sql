...
select dept_name, count(id) count 
from student 
group by dept_name 
having count(id) > 10;

/* 3. Find the total number of courses in each department. */
select dept_name, count(course_id) count 
from course 
group by dept_name;
...
from (
    select dept_name, avg(salary) avgsalary 
    from instructor 
    group by dept_name
) 
...
select dept_name, sum(salary) totalsalary 
from instructor 
group by dept_name
